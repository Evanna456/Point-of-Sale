...
namespace PointOfSale.MainPages
{
    ...
    public partial class MainPage1 : Page
    {
        ...
        private void RegisterTextClick(object sender, RoutedEventArgs e)
        {
...
{ Password = passwordDB }
    .ToString();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        else if (count == "0")
                        {
                            ...
                            NavigationService.Navigate(new Uri("MainPages/MainPage2Register.xaml", UriKind.Relative));
        ...
        public void Login()
        {
            string username, password, count = "0";

            username = Username.Text;
            password = Password.Password.ToString();

            if (Validation("alphanum", "Username", username, 1, 200) != "Validated")
            {
                MessageBox.Show(Validation("variable", "Username", username, 1, 200), "Error");

                Username.BorderBrush = Brushes.Red;
            }
            else if (Validation("alphanum", "Password", password, 1, 200) != "Validated")
            {
                MessageBox.Show(Validation("variable", "Password", password, 1, 200), "Error");
                ...
                .ToString();
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                string password2 = reader.GetString(1);

                                bool passwordstatus = Bcr.BCrypt.Verify(password, password2);

                                if (username == username2 && passwordstatus == true)
                                {
                                    PointofSaleWindow pointofsalewindow = new PointofSaleWindow();
                                    pointofsalewindow.Show();
